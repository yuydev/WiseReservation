...
- **项目名称**：智慧物流云台
- **平台**：微信小程序
- **后端 API 基础地址（仅本地开发示例）**：`https://localhost:7153`（仅适用于本机/模拟器，真机与联调环境必须替换为可访问域名或网关地址）
- **认证方式**：`Authorization: GnBearer {token}`（后端当前定义为自定义前缀，若需对接标准 OAuth2/JWT 中间件可评估迁移为 `Bearer`）
...
> 说明：角色数值来自现有后端/存量系统主数据编码（历史兼容约束），前后端均按该映射进行传输与解析，不按顺序枚举分配。
